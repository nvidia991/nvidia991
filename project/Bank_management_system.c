...
void deposit_money(){
    ...
    scanf("%d",&acc_no);
    printf("Enter amount to dopisit: ");
    scanf("%f",&money);
    ...
    while (fread(&acc_r, sizeof(acc_r), 1, file)) {
     if (acc_r.acc_no == acc_no) {
        ...
        fclose(file);
        printf("Successfully deposite Rs.%.2f New balance is Rs.%.2f",money,acc_r.balance);
        return;
        }
    }
    fclose(file);
    printf("Money could not be depositeded as the Account number %d was not found in records.",acc_no);
}
...
void withdraw_money(){
    ...
    fclose(file);
    printf("Money could not be withdrawn as the Account number %d was not found in records.",acc_no);
}
...
//     struct Account* account = findAccount(accountNumber);
//     if (account != NULL) {
//         printf("\nAccount Details:\n");
